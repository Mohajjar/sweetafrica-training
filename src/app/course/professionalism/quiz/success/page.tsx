...
export default function QuizSuccessPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // Check quiz doc
      const qref = doc(db, "users", u.uid, "quizzes", "professionalism");
      ...
      if (!ok) {
        router.replace("/course/professionalism/quiz");
      }
    });
    return () => unsub();
  }, [router]);
  ...
  const downloadCertificate = () => {
    ...
    docPdf.setFont("helvetica", "bold");
    docPdf.text("Professional Cleaning Service Training", 306, 298, {
      align: "center",
    });
    ...
    docPdf.save("SweetAfrica-Professionalism-Certificate.pdf");
  };
