...
export default function QuizSuccessPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // Check quiz doc
      const qref = doc(db, "users", u.uid, "quizzes", "fundamentals");
      ...
      if (!ok) {
        router.replace("/course/fundamentals/quiz");
      }
    });
    return () => unsub();
  }, [router]);
  ...
  const downloadCertificate = () => {
    ...
    docPdf.setFont("helvetica", "bold");
    docPdf.text("Fundamentals of Cleaning Training", 306, 298, {
      align: "center",
    });
    ...
    docPdf.save("SweetAfrica-Fundamentals-Certificate.pdf");
  };
